...
def add_sparse_arc(graph, head_id, dependent_id, feat_vec):
        ...
        for dependent_id in dependents:
            if dependent_id not in local_dependents and dependent_id != head:
                ...
                if "hform,hpos,dform,dpos:"+hform+","+hpos+","+dform+","+dpos in feat_map:
                    new_feat_vec.append(feat_map["hform,hpos,dform,dpos:"+hform+","+hpos+","+dform+","+dpos])
...
def write_graph_to_file(graph, out_file, mode = "normal"):
    arcs = {}
    if mode == "normal":
        for head in graph:
            for arc in graph[head]:
        ...
        out = open(out_file, "a")
        for arc in sorted(arcs):
            print >> out, str(arcs[arc].dependent) + "\t" + arcs[arc].dependent_form + "\t" + arcs[arc].dependent_lemma + "\t" + arcs[arc].dependent_pos + "\t_\t_\t" + \
                "-1" + "\t" + "__ERROR__" + "\t_\t_"
        print >> out, ""
        out.close()

def check_graph_sanity(compare_graph, predicted_graph):
